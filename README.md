# Hi, I'm Nikushhaa <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=200&text=Nikushhaa&fontSize=70"/>
<img src="https://media.giphy.com/media/qgQUggAC3Pfv687qPC/giphy.gif" width="400"/>
<div align="center">

### 💻 Developer | JavaScript • Python • C# • AI • Web Development

Building interactive applications, automation tools, AI projects, and modern web experiences.

</div>

---

## 👨‍💻 About Me

I'm a developer passionate about creating practical software projects and learning new technologies.

I enjoy building:
<img src="https://media.giphy.com/media/qgQUggAC3Pfv687qPC/giphy.gif" width="400"/>
- 🌐 Modern websites and web applications
- 🤖 AI-powered assistants and automation tools
- 🎮 Interactive games with intelligent systems
- 🖥️ Desktop applications
- ⚡ Projects that combine creativity with programming logic

Currently focusing on improving my skills in:
- JavaScript development
- Python automation
- Artificial Intelligence
- Game development
- Software architecture

---
<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=25&pause=1000&color=00F7FF&center=true&vCenter=true&width=600&lines=Hi+I'm+Nikushhaa;JavaScript+Developer;Python+Developer;AI+Builder;Game+Developer"/>
# 🛠️ Technologies & Tools

### Languages

![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)

![C#](https://img.shields.io/badge/C%23-239120?style=for-the-badge&logo=c-sharp&logoColor=white)

![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)

![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)

+ Lua
### Development Tools

![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)

![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)

![Visual Studio Code](https://img.shields.io/badge/VS_Code-007ACC?style=for-the-badge&logo=visual-studio-code&logoColor=white)


---

# 🚀 Featured Projects

## 🤖 Jarvis Assistant

A Python-based voice-controlled personal assistant designed to automate tasks and interact with users through speech.

**Features:**

- Voice recognition
- Text-to-speech responses
- Computer automation
- Command handling
- Information retrieval

**Technologies:**

`Python` `Speech Recognition` `Automation` `TTS`

---

## 🎮 Reverse Game

A JavaScript implementation of the classic Reversi (Othello) board game with an AI opponent.

**Features:**

- Complete game logic
- Valid move detection
- Automatic piece flipping
- AI decision making
- Board evaluation system

**Technologies:**

`JavaScript` `Algorithms` `Game Logic` `AI`

---

## ⌨️ Typing Speed Website

An interactive typing test application that measures typing speed and accuracy in real time.

**Features:**

- Words per minute calculation
- Accuracy tracking
- Mistake detection
- Timer system
- Dynamic text generation

**Technologies:**

`HTML` `CSS` `JavaScript`

---

## 🌐 Gaming-Hub

A modern gaming website with a responsive interface and interactive elements.

**Features:**

- Responsive design
- Modern UI
- Interactive components
- Gaming-focused layout

**Technologies:**

`HTML` `CSS` `JavaScript`

---

## 🍽️ Restaurant Design

A responsive restaurant website concept focused on modern UI design and user experience.

**Features:**

- Restaurant landing page
- Menu sections
- Responsive layout
- Visual design elements

**Technologies:**

`HTML` `CSS` `JavaScript`

---

# 📚 Currently Learning

- Advanced JavaScript concepts
- Artificial Intelligence
- Better software architecture
- Backend development
- Database systems
- More advanced algorithms

---

# 🎯 Goals

My goal is to continue building useful software, improve my programming skills, and create projects that solve real problems.

I enjoy challenging myself with projects that combine:

- Creativity 🎨
- Logic 🧠
- Technology 🚀

---

# 📊 GitHub Statistics

<div align="center">

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Nikushhaa&show_icons=true&theme=tokyonight)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Nikushhaa&layout=compact&theme=tokyonight)

</div>

---

# 📈 Activity

<div align="center">

![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=Nikushhaa&theme=tokyonight)

</div>

---

# 📫 Contact

Feel free to explore my repositories or connect with me.

**GitHub:**

https://github.com/Nikushhaa

---
**Instagram:**
[https:](https://www.instagram.com/nikushanakashidzee/)
---

<div align="center">

⭐ Thanks for visiting my profile!

</div>
